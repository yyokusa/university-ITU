 ...
 * 
 * @return address of dynamically allocated memory block
 */
char **allocate_mem_for_book()
{
 ...
 * 
 * @param fptr_instructions file pointer which holds a pointer to the "instructions.txt" file
...
char *get_word(char *line, int index, char *word)
{
    ...
    // at the 0th index of the word we want
    int k = word_index;
    int a = 0;
    while (line[k] != ' ')
    {
